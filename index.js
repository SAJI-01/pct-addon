...
function collector(url, next) {
    ...
    needle.get(url, httpOpts, function(err, resp, body) {
        ...
        if (err) { console.error("Request failed:", err.message); return; }
        if (!resp) { console.error("No response"); return; }
        try {
            if (body && body.status && body.data && body.data.movies) {
                console.log("Indexed page", body.data.page_number, "movies:", body.data.movies.length);
...
if (!process.env.DISABLE_IDX) sources.yts.forEach(function(url) { queue.push(url); });
...
setInterval(function() {
    var n = Object.keys(map).filter(function(k) { return k !== "topPages"; }).length;
